...
BOND_TYPES = ["RR", "RD", "DE"]
SYSTEM_PREFIXES = ["AA", "CG", "M3"]

# 如果想手动限制横纵坐标范围，在这里改。None or number
X_AXIS_LIMITS = (0.2, 0.6)
Y_AXIS_LIMITS = None
...
def get_system_style(system_name):
    if system_name == "AA":
        return AA_COLOR, "AA"
    if system_name == "CG":
        return CG_COLOR, "CG"
...
def draw_one_bond_type(script_dir, bond_type):
    ...
    fig.tight_layout()
    output_fig = os.path.join(script_dir, f"bond_{bond_type}_distribution_comparison.svg")
...
def main():
    ...
    for bond_type in BOND_TYPES:
        draw_one_bond_type(script_dir, bond_type)
